...
Player::Player()
{
	...
	m_Speed = Vector2f(1.0f, 1.0f);
	m_Acceleration = Vector2f(10.0f, 10.0f);
	m_FrictionTemp = Vector2f(13.0f, 13.0f);
}
...
void Player::update(sf::RenderWindow& target, float32 dt)
{
	...
	if (!m_Controller.moveUp && !m_Controller.moveDown)
	{
		...
		{
			m_Velocity.y = linearInterpolate(m_Velocity.y, 0.0f, m_FrictionTemp.y * dt);
		}
	}
	else
	{
	...
	if (!m_Controller.moveLeft && !m_Controller.moveRight)
	{
		...
		{
			m_Velocity.x = linearInterpolate(m_Velocity.x, 0.0f, m_FrictionTemp.x * dt);
		}
	}
	else
	{
